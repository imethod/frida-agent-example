...
export class DumpSo {
    ...
    path: string
    libso: Module | null
    constructor(soName: string, path: string, address?: NativePointer) {
        this.soName = soName
        this.path = path
        if (address) {
            this.libso = Process.findModuleByAddress(address)

            logd(this.libso?.name + " " + this.libso?.base)
            logd("" +ptr(this.libso!.size))
        } else {
